...
// Windows) CIMGUI_API is used for core imgui functions, CIMGUI_IMPL_API is used
...
typedef struct ImGuiMultiSelectIO_t
    ImGuiMultiSelectIO; // Structure to interact with a
...
typedef struct ImGuiPlatformIO_t
    ImGuiPlatformIO; // Interface between platform/renderer backends and ImGui
                     // (e.g. Clipboard, IME hooks). Extends ImGuiIO. In docking
                     // branch, this gets extended to support multi-viewports.
typedef struct ImGuiPlatformImeData_t
    ImGuiPlatformImeData; // Platform IME data for io.PlatformSetImeDataFn()
                          // function.
typedef struct ImGuiSelectionBasicStorage_t
    ImGuiSelectionBasicStorage; // Optional helper to store multi-selection
                                // state + apply multi-selection requests.
typedef struct ImGuiSelectionExternalStorage_t
    ImGuiSelectionExternalStorage; // Optional helper to apply multi-selection
                                   // requests to existing randomly accessible
                                   // storage.
...
typedef struct ImGuiStoragePair_t
    ImGuiStoragePair;                   // Helper for key->value storage (pair)
...
typedef int ImGuiDir; // -> enum ImGuiDir              // Enum: A cardinal
...
typedef ImU8 ImGuiSortDirection; // -> enum ImGuiSortDirection    // Enum: A
...
// Main
CIMGUI_API ImGuiIO *
ImGui_GetIO(void); // access the ImGuiIO structure (mouse/keyboard/gamepad
                   // inputs, time, various configuration options/flags)
CIMGUI_API ImGuiPlatformIO *ImGui_GetPlatformIO(
    void); // access the ImGuiPlatformIO structure (mostly hooks/functions to
...
CIMGUI_API bool ImGui_IsWindowHovered(
    ImGuiHoveredFlags
        flags /* = 0 */); // is current window hovered and hoverable (e.g. not
                          ...
                          // this function! Use the 'io.WantCaptureMouse'
                          // boolean for that! Refer to FAQ entry "How can I
                          // tell whether to dispatch mouse/keyboard to Dear
...
CIMGUI_API ImVec2 ImGui_GetWindowPos(
    void); // get current window position in screen space (IT IS UNLIKELY YOU
           // EVER NEED TO USE THIS. Consider always using GetCursorScreenPos()
           // and GetContentRegionAvail() instead)
CIMGUI_API ImVec2 ImGui_GetWindowSize(
    void); // get current window size (IT IS UNLIKELY YOU EVER NEED TO USE THIS.
           // Consider always using GetCursorScreenPos() and
           // GetContentRegionAvail() instead)
CIMGUI_API float ImGui_GetWindowWidth(
    void); // get current window width (IT IS UNLIKELY YOU EVER NEED TO USE
           // THIS). Shortcut for GetWindowSize().x.
CIMGUI_API float ImGui_GetWindowHeight(
    void); // get current window height (IT IS UNLIKELY YOU EVER NEED TO USE
...
CIMGUI_API void ImGui_SetNextWindowSizeConstraints(
    ...
    void *custom_callback_data /* = NULL */); // set next window size limits.
                                              // use 0.0f or FLT_MAX if you
                                              // don't want limits. Use -1 for
                                              // both min and max of same axis
                                              // to preserve current size (which
                                              // itself is a constraint). Use
...
CIMGUI_API void
ImGui_PushStyleVarImVec2(ImGuiStyleVar idx,
                         ImVec2 val); // modify a style ImVec2 variable. "
CIMGUI_API void ImGui_PushStyleVarX(
    ImGuiStyleVar idx,
    float val_x); // modify X component of a style ImVec2 variable. "
CIMGUI_API void ImGui_PushStyleVarY(
    ImGuiStyleVar idx,
...
CIMGUI_API ImVec2 ImGui_GetFontTexUvWhitePixel(
    void); // get UV coordinate for a white pixel, useful to draw custom shapes
...
CIMGUI_API ImU32 ImGui_GetColorU32ImU32Ex(
    ImU32 col,
    float alpha_mul /* = 1.0f */); // retrieve given color with style alpha
                                   // applied, packed as a 32-bit value suitable
                                   // for ImDrawList
...
CIMGUI_API ImVec2 ImGui_GetCursorScreenPos(
    void); // cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND
           // (prefer using this rather than GetCursorPos(), also more useful to
           // work with ImDrawList API).
CIMGUI_API void
ImGui_SetCursorScreenPos(ImVec2 pos); // cursor position, absolute coordinates.
                                      // THIS IS YOUR BEST FRIEND.
CIMGUI_API ImVec2 ImGui_GetContentRegionAvail(
    void); // available space from current position. THIS IS YOUR BEST FRIEND.
CIMGUI_API ImVec2
ImGui_GetCursorPos(void); // [window-local] cursor position in window-local
...
CIMGUI_API ImVec2 ImGui_GetCursorStartPos(
    void); // [window-local] initial cursor position, in window-local
           // coordinates. Call GetCursorScreenPos() after Begin() to get the
...
CIMGUI_API bool
ImGui_SmallButton(const char *label); // button with (FramePadding.y == 0) to
...
CIMGUI_API void
ImGui_Image(ImTextureID user_texture_id,
            ImVec2 image_size); // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1,
                                // 1), tint_col = ImVec4(1, 1, 1, 1), border_col
                                // = ImVec4(0, 0, 0, 0)
CIMGUI_API void ImGui_ImageEx(ImTextureID user_texture_id, ImVec2 image_size,
                              ImVec2 uv0 /* = ImVec2(0, 0) */,
                              ImVec2 uv1 /* = ImVec2(1, 1) */,
                              ImVec4 tint_col /* = ImVec4(1, 1, 1, 1) */,
                              ImVec4 border_col /* = ImVec4(0, 0, 0, 0) */);
CIMGUI_API bool
ImGui_ImageButton(const char *str_id, ImTextureID user_texture_id,
                  ImVec2 image_size); // Implied uv0 = ImVec2(0, 0), uv1 =
                                      // ImVec2(1, 1), bg_col = ImVec4(0, 0, 0,
                                      // 0), tint_col = ImVec4(1, 1, 1, 1)
...
CIMGUI_API bool
ImGui_ComboCallback(const char *label, int *current_item,
                    const char *(*getter)(void *user_data, int idx),
                    void *user_data,
                    int items_count); // Implied popup_max_height_in_items = -1
CIMGUI_API bool ImGui_ComboCallbackEx(const char *label, int *current_item,
...
CIMGUI_API ImGuiMultiSelectIO *
ImGui_BeginMultiSelect(ImGuiMultiSelectFlags flags); // Implied selection_size =
                                                     // -1, items_count = -1
CIMGUI_API ImGuiMultiSelectIO *
ImGui_BeginMultiSelectEx(ImGuiMultiSelectFlags flags,
                         int selection_size /* = -1 */,
                         int items_count /* = -1 */);
CIMGUI_API ImGuiMultiSelectIO *ImGui_EndMultiSelect(void);
CIMGUI_API void
ImGui_SetNextItemSelectionUserData(ImGuiSelectionUserData selection_user_data);
CIMGUI_API bool ImGui_IsItemToggledSelection(
    void); // Was the last item selection state toggled? Useful if you need the
...
CIMGUI_API bool
ImGui_ListBoxCallback(const char *label, int *current_item,
                      const char *(*getter)(void *user_data, int idx),
                      void *user_data,
                      int items_count); // Implied height_in_items = -1
CIMGUI_API bool ImGui_ListBoxCallbackEx(const char *label, int *current_item,
                                        const char *(*getter)(void *user_data,
                                                              int idx),
                                        void *user_data, int items_count,
                                        int height_in_items /* = -1 */);
...
CIMGUI_API bool ImGui_BeginTable(
    const char *str_id, int columns,
    ImGuiTableFlags flags /* = 0 */); // Implied outer_size = ImVec2(0.0f,
                                      // 0.0f), inner_width = 0.0f
CIMGUI_API bool ImGui_BeginTableEx(const char *str_id, int columns,
...
CIMGUI_API void ImGui_TableHeadersRow(
    void); // submit a row with headers cells based on data provided to
           // TableSetupColumn() + submit context menu
CIMGUI_API void ImGui_TableAngledHeadersRow(
    void); // submit a row with angled headers for every column with the
...
CIMGUI_API int ImGui_TableGetHoveredColumn(
    void); // return hovered column. return -1 when table is not hovered. return
           // columns_count if the unused space at the right of visible columns
           // is hovered. Can also use (TableGetColumnFlags() &
...
CIMGUI_API void
ImGui_Columns(void); // Implied count = 1, id = NULL, borders = true
...
CIMGUI_API const ImGuiPayload *ImGui_GetDragDropPayload(
    void); // peek directly into the current payload from anywhere. returns NULL
           // when drag and drop is finished or inactive. use
...
// Focus, Activation
CIMGUI_API void
ImGui_SetItemDefaultFocus(void); // make last item the default focused item of
...
CIMGUI_API bool ImGui_Shortcut(ImGuiKeyChord key_chord,
                               ImGuiInputFlags flags /* = 0 */);
CIMGUI_API void ImGui_SetNextItemShortcut(ImGuiKeyChord key_chord,
                                          ImGuiInputFlags flags /* = 0 */);
...
CIMGUI_API bool ImGui_IsMouseReleasedWithDelay(
    ImGuiMouseButton button,
...
CIMGUI_API bool ImGui_IsMouseDragging(
    ImGuiMouseButton button,
    float lock_threshold /* = -1.0f */); // is mouse dragging? (uses
                                         // io.MouseDraggingThreshold if
                                         // lock_threshold < 0.0f)
CIMGUI_API ImVec2 ImGui_GetMouseDragDelta(
    ...
    float lock_threshold /* = -1.0f */); // return the delta from the initial
                                         ...
                                         // distance threshold at least once
                                         // (uses io.MouseDraggingThreshold if
                                         // lock_threshold < 0.0f)
...
CIMGUI_API void ImGui_DebugLog(const char *fmt, ...) IM_FMTARGS(
...
CIMGUI_API void ImGuiIO_ClearInputKeys(
    ImGuiIO *self); // Clear current keyboard/gamepad state + current frame text
                    // input buffer. Equivalent to releasing all keys/buttons.
CIMGUI_API void
ImGuiIO_ClearInputMouse(ImGuiIO *self); // Clear current mouse state.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
CIMGUI_API void ImGuiIO_ClearInputCharacters(
    ImGuiIO *self); // [Obsoleted in 1.89.8] Clear the current frame text input
...
// ImGuiListClipper::Begin(INT_MAX) if you don't know item count ahead of time.
...
CIMGUI_API void ImGuiSelectionBasicStorage_ApplyRequests(
    ImGuiSelectionBasicStorage *self,
    ImGuiMultiSelectIO
        *ms_io); // Apply selection requests coming from BeginMultiSelect() and
                 // EndMultiSelect() functions. It uses 'items_count' passed to
                 // BeginMultiSelect()
...
CIMGUI_API void ImGuiSelectionBasicStorage_SetItemSelected(
    ImGuiSelectionBasicStorage *self, ImGuiID id,
...
CIMGUI_API bool ImGuiSelectionBasicStorage_GetNextSelectedItem(
    ImGuiSelectionBasicStorage *self, void **opaque_it,
    ImGuiID
        *out_id); // Iterate selection with 'void* it = NULL; ImGuiID id; while
                  // (selection.GetNextSelectedItem(&it, &id)) { ... }'
...
CIMGUI_API void ImDrawList_AddEllipse(
    ImDrawList *self, ImVec2 center, ImVec2 radius,
    ImU32 col); // Implied rot = 0.0f, num_segments = 0, thickness = 1.0f
CIMGUI_API void ImDrawList_AddEllipseEx(ImDrawList *self, ImVec2 center,
                                        ImVec2 radius, ImU32 col,
                                        float rot /* = 0.0f */,
                                        int num_segments /* = 0 */,
                                        float thickness /* = 1.0f */);
CIMGUI_API void
ImDrawList_AddEllipseFilled(ImDrawList *self, ImVec2 center, ImVec2 radius,
                            ImU32 col); // Implied rot = 0.0f, num_segments = 0
CIMGUI_API void ImDrawList_AddEllipseFilledEx(ImDrawList *self, ImVec2 center,
                                              ImVec2 radius, ImU32 col,
                                              float rot /* = 0.0f */,
                                              int num_segments /* = 0 */);
CIMGUI_API void
...
CIMGUI_API void ImDrawList_AddTextImFontPtr(
    ImDrawList *self, ImFont *font, float font_size, ImVec2 pos, ImU32 col,
    const char *text_begin); // Implied text_end = NULL, wrap_width = 0.0f,
                             // cpu_fine_clip_rect = NULL
CIMGUI_API void
ImDrawList_AddTextImFontPtrEx(ImDrawList *self, ImFont *font, float font_size,
                              ImVec2 pos, ImU32 col, const char *text_begin,
                              const char *text_end /* = NULL */,
...
CIMGUI_API void
ImDrawList_PathEllipticalArcTo(ImDrawList *self, ImVec2 center, ImVec2 radius,
                               float rot, float a_min,
                               float a_max); // Implied num_segments = 0
CIMGUI_API void
ImDrawList_PathEllipticalArcToEx(ImDrawList *self, ImVec2 center, ImVec2 radius,
...
// - Use special ImDrawCallback_ResetRenderState callback to instruct backend to
// reset its render state to the default.
// - Your rendering loop must check for 'UserCallback' in ImDrawCmd and call the
// function instead of rendering triangles. All standard backends are honoring
// this.
...
// - IMPORTANT: please be mindful of the different level of indirection between
// using size==0 (copying argument) and using size>0 (copying pointed data into
// a buffer).
//   - If userdata_size == 0: we copy/store the 'userdata' argument as-is. It
//   will be available unmodified in ImDrawCmd::UserCallbackData during render.
//   - If userdata_size > 0,  we copy/store 'userdata_size' bytes pointed to by
//   'userdata'. We store them in a buffer stored inside the drawlist.
...
CIMGUI_API void
ImDrawList_AddCallback(ImDrawList *self, ImDrawCallback callback,
                       void *userdata); // Implied userdata_size = 0
CIMGUI_API void ImDrawList_AddCallbackEx(ImDrawList *self,
...
CIMGUI_API ImFont *ImFontAtlas_AddFontFromMemoryTTF(
    ImFontAtlas *self, void *font_data, int font_data_size, float size_pixels,
    const ImFontConfig *font_cfg /* = NULL */,
    const ImWchar
...
CIMGUI_API void ImFontAtlas_ClearFonts(
    ImFontAtlas *self); // Clear input+output font data (same as
...
struct ImFont_t {
  ...
  ImVector_float IndexAdvanceX; // 12-16 // out // Sparse. Glyphs->AdvanceX in a
  ...
  ImVector_ImU16 IndexLookup; // 12-16 // out // Sparse. Index glyphs by Unicode
                              // code-point.
...
CIMGUI_API ImVec2 ImFont_CalcTextSizeA(
    ImFont *self, float size, float max_width, float wrap_width,
...
CIMGUI_API ImVec2 ImGui_GetContentRegionMax(
    void); // Content boundaries max (e.g. window boundaries including
...
CIMGUI_API ImVec2 ImGui_GetWindowContentRegionMin(
    void); // Content boundaries min for the window (roughly (0,0)-Scroll), in
...
CIMGUI_API ImVec2 ImGui_GetWindowContentRegionMax(
    void); // Content boundaries max for the window (roughly (0,0)+Size-Scroll),
           // in window-local coordinates. You should never need this. Always
           // use GetCursorScreenPos() and GetContentRegionAvail()!
// OBSOLETED in 1.90.0 (from September 2023)
CIMGUI_API bool ImGui_BeginChildFrame(ImGuiID id,
                                      ImVec2 size); // Implied window_flags = 0
CIMGUI_API bool
ImGui_BeginChildFrameEx(ImGuiID id, ImVec2 size,
                        ImGuiWindowFlags window_flags /* = 0 */);
...
CIMGUI_API bool ImGui_ComboObsolete(
    ...
    void *user_data, int items_count); // Implied popup_max_height_in_items = -1
...
CIMGUI_API bool ImGui_ListBoxObsolete(
    const char *label, int *current_item,
    bool (*old_callback)(void *user_data, int idx, const char **out_text),
    void *user_data, int items_count); // Implied height_in_items = -1
